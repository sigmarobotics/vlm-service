# Real Integration Test Script — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Build `scripts/real_test.py` — a CLI script that verifies all vlm-service modules with real Gemini API calls and real patrol photos/videos.

**Architecture:** Single Python script with argparse (`--module`, `--all`, `--api-key`). Five test functions (schema, prompt, inspect, video, report), each self-contained. No test framework — just print output for human review.

**Tech Stack:** Python 3.10+, argparse, vlm_service (local package), google-genai (for real API calls)

**Test Data (absolute paths, already on disk):**
- NG photo: `/home/snaken/CodeBase/visual-patrol/data/robot-a/report/images/42_20260213_093558/__1_NG_ffb8d5f8-facf-4c3e-85a2-2c67396f9111.jpg`
- OK photo: `/home/snaken/CodeBase/visual-patrol/data/robot-a/report/images/42_20260213_093558/AED1_OK_889d98cf-4de1-43d0-9a4f-35047b4d3a98.jpg`
- Video: `/home/snaken/CodeBase/visual-patrol/data/robot-a/report/video/44_20260213_234753.mp4`

---

### Task 1: Script Skeleton + Schema Module

**Files:**
- Create: `scripts/real_test.py`

**Step 1: Create the script with argparse + schema module**
...
def test_prompt():
    ...
    for name, config in ALL_CONFIGS.items():
        ...
        print(prompt)
        assert f"[Role]" in prompt, "FAIL: [Role] section missing"
        assert f"[Language]" in prompt, "FAIL: [Language] section missing"
...
def test_inspect(api_key: str):
    ...
    provider.configure(api_key=api_key, model="gemini-2.0-flash")
    ...
    for label, photo_path in [("NG", NG_PHOTO), ("OK", OK_PHOTO)]:
        ...
        result = provider.generate_inspection(
            image=image_bytes,
            user_prompt="この場所を点検してください。" if HOSPITAL_CONFIG["language"] == "ja" else "請檢查這個區域。",
            schema=schema,
...
**Step 2: Run schema + prompt modules to verify (no API needed)**

Run: `python3 scripts/real_test.py --module schema`
Run: `python3 scripts/real_test.py --module prompt`

**Step 3: Run inspect module with real API**

Run: `python3 scripts/real_test.py --module inspect --api-key AIzaSyD780FMHpPirlI2N1WiQABb2_JCJRrW7Ho`

**Step 4: Run video module with real API**

Run: `python3 scripts/real_test.py --module video --api-key AIzaSyD780FMHpPirlI2N1WiQABb2_JCJRrW7Ho`

**Step 5: Run report module with real API**

Run: `python3 scripts/real_test.py --module report --api-key AIzaSyD780FMHpPirlI2N1WiQABb2_JCJRrW7Ho`

**Step 6: Run all modules together**

Run: `python3 scripts/real_test.py --all --api-key AIzaSyD780FMHpPirlI2N1WiQABb2_JCJRrW7Ho`

**Step 7: Commit**

```bash
git add scripts/real_test.py
git commit -m "test: real integration test script with module selection"
```
